...
$sizes = [
    'sm' => 'p-2 text-sm',
    'md' => 'p-4 text-base', // Reduced padding for default size
    'lg' => 'p-6 text-lg',
];
...
$roundedClass = 'rounded-lg shadow-md'; // Consistent rounded corners and shadow for cards
@endphp

<div {{ $attributes->merge(['class' => "$sizeClass $colorClass $roundedClass"]) }}>
    @if ($header)
        <div class="flex items-center border-b border-gray-200 dark:border-gray-700 px-4 py-2">
            @if ($icon)
                <x-slate::icon :icon="$icon" size="md" class="mr-2"/>
            @endif
            <div class="flex-1">
                {{ $header }}
            </div>
