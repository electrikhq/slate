...
@php
    // Base classes - radio button styling (rounded-full for radio vs rounded-sm for checkbox)
    $baseClasses = 'h-4 w-4 shrink-0 rounded-full border border-primary ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition-colors';
    
    // Size classes - radio size variants
    $sizeClasses = [
        'sm' => 'h-3.5 w-3.5',
        'default' => 'h-4 w-4',
        'lg' => 'h-5 w-5',
    ...
    $radioId = $id ?? ($name && $value ? "radio-{$name}-{$value}" : null);
    ...
    if ($hasError && $radioId) {
        ...
        $describedBy[] = "{$radioId}-error";
    }
    
    if ($help && $radioId) {
        $describedBy[] = "{$radioId}-help";
    }
    ...
    if (!$isChecked && $name && function_exists('old') && !$isLivewire) {
        $oldValue = old($name);
        // For radio buttons, old() returns the value if this option was selected
        $isChecked = $oldValue !== null && $oldValue !== false && $oldValue !== '' && (string)$oldValue === (string)$value;
    }
...
<div class="flex flex-col items-start space-y-1">
    ...
    @if($help && $radioId)
        <p id="{{ $radioId }}-help" class="text-sm text-muted-foreground ml-6">
            {{ $help }}
        </p>
    @endif
    
    @if($hasError && $finalErrorMessage && $radioId)
        <p id="{{ $radioId }}-error" class="text-sm text-danger ml-6">
            {{ $finalErrorMessage }}
        </p>
    @endif
</div>
