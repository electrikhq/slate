@props([
    ...
    'color' => 'primary', // Color of the dropdown trigger button
...
if (in_array($color, ['black', 'white'])) {
    ...
    $colorClass = "bg-{$color}-500 text-white dark:bg-{$color}-700 dark:text-neutral-200";
...
<div x-data="{ open: false }" class="relative inline-block text-left">
    ...
    <button @click="open = !open" :class="{ 'opacity-50 cursor-not-allowed': {{ $disabled ? 'true' : 'false' }} }"
            {{ $disabled ? 'disabled' : '' }}
            class="inline-flex items-center justify-between {{ $sizeClass }} {{ $colorClass }} rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-neutral-700">
        @if($icon)
    ...
    <div x-show="open" @click.away="open = false" 
        class="absolute z-10 {{ $positionClasses }} min-w-[100%] bg-white dark:bg-black border border-neutral-300 dark:border-neutral-700 rounded-md {{ $shadowClass }} overflow-hidden">
